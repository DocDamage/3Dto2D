...
Adds product-level helpers around the core WAN/sprite pipeline:
- next-step recommendations
- all-in-one preflight report
- standalone asset dashboard
- release package builder for finished sprites/projects

Everything here is dependency-light and safe to run before ComfyUI is installed.
"""
...
def selected_sprite_dirs(args: argparse.Namespace) -> List[Path]:
    ...
    if args.project:
        ...
        if not pr.is_absolute():
            pr = ROOT / pr
        # Include project-local sprites plus outputs matching project name.
